...
EncButton2<EB_BTN> enc(INPUT_PULLUP, 4);  

const char* ssid = "Kapella";
const char* password = "serayakoshechka";

const char* mqttserver = "130.193.44.244"; //mqtt.cloud.yandex.net
const char* yandexIoTCoredeviceId = "arepl2dd34v01v9p3577";
const char* mqttpassword = "MIIFGTCCAwGgAwIBAgIQJMM7ZIy2";
//MIIFGTCCAwGgAwIBAgIQJMM7ZIy22 пароль реестра
...
5N6xOftRrCpy8hGtUR/A3bvjnQgjs+zdXvcO9cTuufffffffffffffffzzPTFSts/iZATZsAruiepMx\
...
enc.tick(); 
 if (enc.click()) {
      if (client.publish(topicEvents.c_str(), "hello from ESP")) {
      DEBUG_SERIAL.println("Publish ok");
    }
    else {
      DEBUG_SERIAL.println("Publish failed");
